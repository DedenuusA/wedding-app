    ...
    <div class="p-6 max-w-6xl mx-auto">
        ...
        <form method="POST" action="{{ route('wedding.update') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <input type="text" name="bride_name" value="{{ $wedding->bride_name ?? '' }}"
                    placeholder="Nama Mempelai Wanita" class="border rounded p-3 w-full">

                <input type="text" name="groom_name" value="{{ $wedding->groom_name ?? '' }}"
                    placeholder="Nama Mempelai Pria" class="border rounded p-3 w-full">

                <input type="text" name="bride_parent" value="{{ $wedding->bride_parent ?? '' }}"
                    placeholder="Orang Tua Wanita" class="border rounded p-3 w-full">

                <input type="text" name="groom_parent" value="{{ $wedding->groom_parent ?? '' }}"
                    placeholder="Orang Tua Pria" class="border rounded p-3 w-full">

                <input type="date" name="date" value="{{ $wedding->date ?? '' }}"
                    class="border rounded p-3 w-full">

                <input type="time" name="time" value="{{ $wedding->time ?? '' }}"
                    class="border rounded p-3 w-full">

                <input type="text" name="location" value="{{ $wedding->location ?? '' }}" placeholder="Lokasi"
                    class="border rounded p-3 w-full">

                <input type="text" name="maps_link" value="{{ $wedding->maps_link ?? '' }}"
                    placeholder="Google Maps Link" class="border rounded p-3 w-full">

            </div>

            <textarea name="story" placeholder="Cerita cinta" class="border rounded p-3 w-full mt-4 h-32">{{ $wedding->story ?? '' }}</textarea>

            <select name="theme" class="border rounded p-3 w-full mt-4">
                ...
                <option value="invits" {{ optional($wedding)->theme == 'invits' ? 'selected' : '' }}>
                    Invits
                </option>
            ...
            <div class="mt-6">
                ...
                @if (optional($wedding)->qris)
                    <img src="{{ asset('storage/' . $wedding->qris) }}" class="w-40 mt-2 rounded shadow">
                @endif
            </div>
            ...
            <div class="mt-6">
                ...
                <div id="bank-wrapper">
                    @foreach ($wedding->banks as $bank)
                        <div class="bank-item border p-3 mb-2 rounded relative">
                            ...
                            <button type="button" onclick="removeBank(this)"
                            ...
                            <div class="bank-select-wrapper relative w-64 mt-6">
                                <div class="selected flex items-center border p-2 cursor-pointer">
                                    <img src="{{ asset('images/' . $bank->masterbank->logo ?? '') }}"
                                        class="bank-logo w-10 h-10 object-contain mr-2" style="display:block;">
                                    <span class="bank-name">{{ $bank->bank_name }}</span>
                                </div>

                                <div
                                    class="options absolute border bg-white w-full mt-1 hidden z-10 max-h-40 overflow-auto">
                                    @foreach ($banks as $b)
                            ...
                            <input type="text" name="banks[number][]" value="{{ $bank->account_number }}"
                                placeholder="No Rekening" class="border p-2 w-full mb-1 mt-2">

                            <input type="text" name="banks[holder][]" value="{{ $bank->account_holder }}"
                                placeholder="Atas Nama" class="border p-2 w-full">
                ...
                <button type="button" onclick="addBank()" class="bg-gray-800 text-white px-4 py-2 rounded mb-4">
                    + Tambah Bank
                </button>


                <button
                    class="mt-8 bg-rose-600 hover:bg-rose-700 text-white px-6 py-3 rounded shadow w-full md:w-auto">
                    Save Wedding
                </button>
    ...
    <div class="bank-item border p-3 mb-2 rounded relative">
        ...
        document.addEventListener('click', e => {
            if (!e.target.closest('.bank-select-wrapper')) {
                document.querySelectorAll('.bank-select-wrapper .options').forEach(opt => opt.classList.add(
                    'hidden'));
            }
        });
